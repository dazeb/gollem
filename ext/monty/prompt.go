...
func buildSystemPrompt(toolName string, tools map[string]*core.Tool, schemas map[string]core.Schema) string {
	...
	b.WriteString("- Supported Python: loops, comprehensions, f-strings, lambdas, closures, generators, try/except, ")
	b.WriteString("PEP 448 unpacking (`*args`, `**kwargs`), tuple comparison, nested/augmented subscript assignment, multi-module imports (`import a, b, c`)\n")
	b.WriteString("- Supported builtins: `filter`, `getattr`, `max`, `min`, `sum`, `sorted`, `reversed`, `enumerate`, `zip`, `map`, `all`, `any`, `isinstance`, `type`, `int`, `float`, `str`, `bool`, `abs`, `len`, `range`, `print`\n")
	b.WriteString("- Supported stdlib modules: `math` (full), `re`, `datetime`, `json`; plus `import os` and `from pathlib import Path` via OS callbacks\n")
	b.WriteString("- Not supported in execute_code: `class` definitions (only dataclass instances via external I/O), `match` statements, context managers (`with ...`), third-party libraries, `float('inf')`, `float('nan')`\n")
	b.WriteString("- If you hit ImportError/ModuleNotFoundError for a module outside the supported set, switch to `bash` with `python3 - <<'PY' ... PY`\n")
