...
func (m model) View() string {
	...
	availableLines := visibleLineCount(m.height)
...
func (m model) renderSplitView(availableLines int) string {
	...
	detailWidth := m.width - timelineWidth - separatorWidth
	if detailWidth < 40 {
		detailWidth = 40
		timelineWidth = m.width - separatorWidth - detailWidth
	}
	if timelineWidth < 24 {
		timelineWidth = 24
		detailWidth = m.width - separatorWidth - timelineWidth
	}
	if detailWidth < 1 {
		detailWidth = 1
	}
	...
	for idx, s := range visible {
		...
		line := prefix + singleLine(stepLabel(s))
		line = padRightDisplay(line, timelineWidth)
		leftLines = append(leftLines, stepStyle(s, m.theme).Render(line))
	}
	for len(leftLines) < availableLines {
		leftLines = append(leftLines, strings.Repeat(" ", timelineWidth))
	}

	rightLines := splitDetailLines(m.steps[selected], detailWidth, availableLines)
	var sb strings.Builder
	for i := range availableLines {
		left := strings.Repeat(" ", timelineWidth)
		if i < len(leftLines) {
			left = leftLines[i]
		}
		right := ""
		if i < len(rightLines) {
			right = truncateDisplay(rightLines[i], detailWidth)
		}
		sb.WriteString(left)
...
func stepLabel(s step) string {
	...
	case "model":
		return "[model] " + s.content
	case "tool-call":
		return "[tool] " + s.content
	case "tool-result":
		return "[result] " + s.content
	case "error":
		return "[error] " + s.content
	case "done":
		return "[done] " + s.content
	case "checkpoint":
		return "[checkpoint] " + s.content
	default:
		return s.content
	}
}

func stepStyle(s step, theme Theme) lipgloss.Style {
	switch s.kind {
...
func renderStep(s step, theme Theme, width int) string {
	...
	return stepStyle(s, theme).Width(maxWidth).Render(stepLabel(s))
}
...
func (m model) renderStatusBar() string {
	...
	status := truncateDisplay(singleLine(FormatTraceStatusBar(m.usage, m.status, m.cost, m.elapsed, modeStr)), innerWidth)
	return m.theme.Status.Width(barWidth).Render(status)
}
...
func FormatTraceStatusBar(usage core.RunUsage, status, cost, elapsed, mode string) string {
	var prefix []string
	if status != "" {
		prefix = append(prefix, "status: "+status)
	}
	...
	return fmt.Sprintf(
		"%sn/p:step g/G:jump e:error c:ckpt d:diverge q:quit | tokens: %d/%d | requests: %d | tools: %d | mode: %s",
		lead,
		usage.InputTokens, usage.OutputTokens,
		usage.Requests, usage.ToolCalls,
		mode,
...
func scrollToFirstKind(steps []step, height int, kind string) int {
	for idx, step := range steps {
		...
		visibleLines := visibleLineCount(height)
...
func takeDisplayWidth(value string, width int) string {
	if width <= 0 {
		return ""
	}
	var b strings.Builder
