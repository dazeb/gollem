...
func (r *RuntimeRecorder) recordArtifactChanged(ev core.ArtifactChangedEvent) {
	r.append(Event{
		...
		Payload: compactMap(map[string]any{
			...
			"before_content":         ev.BeforeContent,
			"after_content":          ev.AfterContent,
			"content_encoding":       ev.ContentEncoding,
			"content_truncated":      ev.ContentTruncated,
			"content_omitted_reason": ev.ContentOmittedReason,
		}),
	})
}
