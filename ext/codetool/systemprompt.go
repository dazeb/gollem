...
14. **Black-box parameter extraction**: When extracting parameters from a black-box function (e.g., a neural network), your solution MUST work purely through input/output queries — do NOT rely on module introspection (` + "`vars()`" + `, ` + "`__dict__`" + `, attribute access) because verifiers replace the module with implementations where parameters are hidden as local variables. For ReLU networks, determine the hidden dimension dynamically by probing (e.g., counting activation boundary changes), then extract weight rows one at a time. Never hardcode the hidden dimension from the development model. After extraction, VERIFY by reconstructing forward(x) from your recovered parameters on random inputs — if max absolute error is not near-zero, you missed neurons. Use aggressive oversampling (many random probe directions) to ensure all neurons are found, especially since the test model may have more neurons than the development model.`
