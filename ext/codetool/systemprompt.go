...
11. **Hardcoding discovered values**: If a task says a parameter is unknown (e.g., "you don't know the shape"), your solution MUST discover it dynamically — even if you can read the source and see the value. Verifiers often swap the underlying data/model/parameters at test time to ensure your solution is general. Reading source code for understanding the problem is fine, but hardcoding values you find there (dimensions, seeds, thresholds, file paths) will fail when the verifier changes them. Build adaptive solutions that auto-detect parameters.
...
13. **Polyglot programs (single file, multiple languages)**: When writing a file that must compile/run as two different languages, exploit asymmetries between the languages: (a) comment syntax differences — e.g., Rust supports nested block comments (` + "`/* /* */ */`" + ` is valid) while C/C++ does not, enabling comment-based code toggling where one compiler sees code and the other sees a comment; (b) preprocessor directives — C/C++ ` + "`#define`" + ` can redefine keywords from another language (` + "`fn`" + `, ` + "`let`" + `, ` + "`println`" + `) into no-ops or C++ equivalents; (c) string literal differences — Python triple-quotes can hide C code, and ` + "`#`" + ` is a comment in both Python and C preprocessor. Write the shared algorithm once, and use these mechanisms to hide language-specific boilerplate from each compiler/interpreter. Test with BOTH compilers/interpreters after every change.`
