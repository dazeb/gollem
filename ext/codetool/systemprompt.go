...
- **lsp**: Use for semantic code navigation when available. Methods: definition (go to definition), references (find all usages), hover (type info), diagnostics (errors), symbols (search by name), rename (rename symbol across workspace), outline (list all symbols in a file). Use rename for safe multi-file refactoring instead of grep+edit. Use outline to understand file structure without reading the whole file. Supports Go, Python, TypeScript/JS, Rust, C/C++, Java, Ruby, Haskell, Zig, Kotlin, Swift, Elixir, Scala, PHP, Dart, OCaml, Lua, C#, Erlang, Nim, Crystal. Requires a language server installed (gopls, pyright, typescript-language-server, etc). Falls back gracefully if unavailable — use grep/view instead.
