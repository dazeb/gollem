...
`typescript/testdata/user_input_wire_v1.ts`. File-change item, dynamic tool-call,
and user-input variants also have strict negative compile contracts. Tests
...
runtime items, daemon status, initialization, thread discovery, thread controls,
thread goals, metadata, lifecycle notifications, command-exec controls, and file
changes.
Dynamic client-tool and structured user-input requests/responses are covered by
the same floor and strict fixtures.
