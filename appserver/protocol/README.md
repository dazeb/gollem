...
`typescript/testdata/dynamic_tool_call_wire_v1.ts`. File-change item and dynamic
tool-call variants also have strict negative compile contracts. Tests compare
generated bytes with the checked-in files, verify every type binding references
a known method and definition, and enforce a compatibility floor for runtime items,
...
Dynamic client-tool requests and responses are covered by the same floor and
strict fixtures.
...
optional Gollem v1 request metadata, and text/image response variants. Consumers
