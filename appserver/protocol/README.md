...
`typescript/testdata/runtime_wire_v1.ts`. Tests compare generated bytes with
the checked-in files, verify every type binding references a known method and
definition, and enforce a compatibility floor for runtime items and daemon
status.
...
When TypeScript is installed, verify the generated binding and v1 fixture with:
