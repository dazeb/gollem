...
func TestAccountEnvelopeContractsRemainStandaloneAndDeferred(t *testing.T) {
	...
	if len(Methods()) != 224 || len(WireTypeBindings()) != 71 || len(ItemPayloadBindings()) != 5 {
		t.Fatalf("counts = %d methods/%d method bindings/%d item bindings; want 224/71/5", len(Methods()), len(WireTypeBindings()), len(ItemPayloadBindings()))
	}
}
