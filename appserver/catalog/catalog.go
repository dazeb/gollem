...
func builtinTools(services ToolServices) []Tool {
	return []Tool{
		...
		tool("file-change-revert", "File change revert", "Restore one exact workspace file state from durable private recovery evidence.", "workspace", []string{"item/fileChange/revert"}, services.Filesystem, true, true, "fs/changed", true, false),
