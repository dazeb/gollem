...
func builtinTools(services ToolServices) []Tool {
	return []Tool{
		tool("fs", "Filesystem", "Read, write, copy, remove, and inspect files under the configured workspace root.", "workspace", []string{"fs/readFile", "fs/writeFile", "fs/createDirectory", "fs/readDirectory", "fs/getMetadata", "fs/remove", "fs/copy"}, services.Filesystem, true, true, "fs/changed", true, false),
